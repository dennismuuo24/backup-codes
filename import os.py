...
class DashboardFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        background_image = Image.open("4.png")
        ...
        self.logo_image = tk.PhotoImage(file="imagess.png")  # Replace "imagess.png" with your logo file
    ...
    def open_chatbot_window(self):
        ...
        chatbot_window.title("Chatbot Conversation")

        # Create a ScrolledText widget to display the chat conversation
        self.chat_conversation = scrolledtext.ScrolledText(chatbot_window, width=50, height=20)
        self.chat_conversation.pack(expand=True, fill=tk.BOTH)

        # Create an Entry widget for user input
        self.user_input_entry = tk.Entry(chatbot_window, width=50)
        self.user_input_entry.pack(side=tk.LEFT, padx=10, pady=10, expand=True, fill=tk.BOTH)

        # Create a Send button to send user input
        send_button = tk.Button(chatbot_window, text="Send", command=self.send_user_input)
    ...
    def send_user_input(self):
        ...
        if user_input:
            # Append the user input to the chat conversation
            self.chat_conversation.insert(tk.END, f"User: {user_input}\n")
            self.chat_conversation.yview(tk.END)  # Auto-scroll to the end

            # Process user input (send to chatbot API, etc.)
